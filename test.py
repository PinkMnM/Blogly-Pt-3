...
class FlaskTests(TestCase):
    ...
    def test_new_user_ajax(self):
        ...
        with app.test_client() as client:
            ...
            self.assertEqual(len(response.json["errors"]), 1)
            self.assertEqual(
                response.json["errors"][0], requires_nonwhitespace_chars("first_name")
            )
            ...
            self.assertEqual(len(response.json["errors"]), 1)
            self.assertEqual(
                response.json["errors"][0], requires_nonwhitespace_chars("last_name")
            )
            ...
            self.assertIn(
                requires_nonwhitespace_chars("first_name"), response.json["errors"]
            )
            self.assertIn(
                requires_nonwhitespace_chars("last_name"), response.json["errors"]
            )
            ...
            self.assertIsNotNone(response.json.get("errors"))
            self.assertEqual(len(response.json["errors"]), 2)
            self.assertIn(missing_parameter("first_name"), response.json["errors"])
            self.assertIn(
                requires_nonwhitespace_chars("last_name"), response.json["errors"]
            )
    ...
    def test_edit_user_ajax(self):
        ...
        with app.test_client() as client:
            ...
            response = client.patch(
                f"/users/{first_user.id}", json={"first_name": first_user.first_name}
            )
            self.assertEqual(response.status_code, 200)
            juser = client.get(
                f"/users/{first_user.id}", headers={"accept": "application/json"}
            ).json["user"]
            ...
            response = client.patch(
                f"/users/{first_user.id}", json={"last_name": first_user.last_name}
            )
            self.assertEqual(response.status_code, 200)
            juser = client.get(
                f"/users/{first_user.id}", headers={"accept": "application/json"}
            ...
            response = client.patch(
                f"/users/{first_user.id}", json={"first_name": "  ", "last_name": ""}
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(response.json["type"], "error")
            self.assertEqual(len(response.json["errors"]), 2)
    ...
    def test_new_post_ajax(self):
        ...
        with app.test_client() as client:
            ...
            first_user = User.query.first()
            response = client.post(
                f"/users/{first_user.id}/posts/new",
                headers={"accept": "application/json"},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(response.json["type"], "error")
            self.assertEqual(len(response.json["errors"]), 2)
            self.assertIn(missing_parameter("title"), response.json["errors"])
            self.assertIn(missing_parameter("content"), response.json["errors"])
    ...
    def test_get_post_ajax(self):
        ...
        with app.test_client() as client:
            response = client.get("/posts/1000", headers={"accept": "application/json"})
            self.assertEqual(response.status_code, 404)
            self.assertEqual(response.json["type"], "error")
            self.assertEqual(len(response.json["errors"]), 1)
            ...
            response = client.get(
                f"/posts/{post_id}", headers={"accept": "application/json"}
            )
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json["type"], "success")
    ...
    def test_get_posts_ajax(self):
        ...
        with app.test_client() as client:
            response = client.get(
                "/users/1000/posts", headers={"accept": "application/json"}
            )
            self.assertEqual(response.status_code, 404)
            self.assertEqual(response.json["type"], "error")
            self.assertEqual(len(response.json["errors"]), 1)
            ...
            first_user = User.query.first()
            response = client.get(
                f"/users/{first_user.id}/posts", headers={"accept": "application/json"}
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_edit_post_ajax(self):
        ...
        with app.test_client() as client:
            ...
            response = client.patch(f"/posts/{post_id}", json={"title": ""})
            self.assertEqual(response.status_code, 400)
            self.assertEqual(response.json["type"], "error")
            self.assertEqual(len(response.json["errors"]), 1)
            self.assertEqual(
                response.json["errors"][0], requires_nonwhitespace_chars("title")
            )

            response = client.patch(f"/posts/{post_id}", json={"content": ""})
            self.assertEqual(response.status_code, 400)
            self.assertEqual(response.json["type"], "error")
            self.assertEqual(len(response.json["errors"]), 1)
            self.assertEqual(
                response.json["errors"][0], requires_nonwhitespace_chars("content")
            )

            content = "New string"
            response = client.patch(f"/posts/{post_id}", json={"content": content})
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json["type"], "success")
